...
2. Codex では `AGENTS.md` `.codex/README.md` `.codex/config.toml.example` を優先する
3. Claude Code では `.claude/CLAUDE.md` `.claude/settings.json` `.claude/commands/` を優先する
...
- Claude Code の共有権限ルールは `.claude/settings.json`、作業方針は `.claude/CLAUDE.md`、定型作業は `.claude/commands/` に寄せる
